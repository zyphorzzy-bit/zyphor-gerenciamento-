...
// ID Atualizado do Bot Gerenciador
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.customId.startsWith('adm_')) {
        ...
        if (interaction.customId === 'adm_registrar_bot') {
            return interaction.reply({
                content: `${e.verde} Módulo de Registro selecionado. Envie os dados do cliente para registrar.`,
                ephemeral: true
            });
        }
        ...
        if (interaction.customId === 'adm_gerenciar_tempo') {
            return interaction.reply({
                content: `${e.horario} Módulo de Gerenciamento de Tempo/Licença selecionado.`,
                ephemeral: true
            });
        }

        // --- RESPOSTA DO BOTÃO REMOVER ---
        if (interaction.customId === 'adm_remover_bot') {
            return interaction.reply({
                content: `${e.vermelho} Módulo de Remoção selecionado. Selecione a licença ou insira o ID da conta/bot para deletar do banco de dados.`,
                ephemeral: true
            });
        }
    }
